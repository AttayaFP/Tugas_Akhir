...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-black text-slate-800 tracking-tight">Tambah Layanan</h1>
                <p class="text-sm font-medium text-slate-500 mt-1">Buat tawaran jasa cetak baru untuk pelanggan.</p>
            </div>
        </div>
    </div>

    <div class="max-w-4xl">
        <form id="main-form" action="{{ route('layanan.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-12 gap-12">
            @csrf
            ...
            <div class="lg:col-span-4">
                <div class="glass rounded-[2.5rem] p-8 border border-white/60 shadow-2xl shadow-slate-200/40 sticky top-28">
                    ...
                    <div class="relative group cursor-pointer">
                        <div id="image-preview" class="w-full aspect-square rounded-[2rem] bg-slate-50 border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-300 overflow-hidden transition-all group-hover:border-[var(--theme-primary)] group-hover:bg-white">
                            <i data-lucide="image-plus" class="w-12 h-12 mb-3"></i>
                            <span class="text-[10px] font-bold">KLIK UNTUK UNGGAH</span>
                        </div>
                    ...
                    <p class="text-[10px] font-medium text-slate-400 mt-6 leading-relaxed">
                        ...
                        Gunakan gambar berkualitas tinggi (JPG/PNG, Maks. 2MB) untuk daya tarik maksimal.
                    </p>
                </div>
            </div>
                ...
                <div class="glass rounded-[2.5rem] p-10 border border-white/60 shadow-2xl shadow-slate-200/40">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="nama_layanan" value="{{ old('nama_layanan') }}" required placeholder="Contoh: Cetak Spanduk High-Res"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="harga_per_satuan" value="{{ old('harga_per_satuan') }}" required placeholder="0"
                        ...
                        <div>
                            ...
                            <input type="text" name="satuan" value="{{ old('satuan') }}" required placeholder="Contoh: Meter, Pcs, Rim"
                                class="w-full glass border border-white/40 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 focus:outline-none focus:ring-4 focus:ring-[var(--theme-accent)] focus:border-[var(--theme-primary)] transition-all">
                        ...
                        <div>
                            ...
                            <input type="number" name="minimal_order" value="{{ old('minimal_order', 1) }}" required
                                class="w-full glass border border-white/40 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 focus:outline-none focus:ring-4 focus:ring-[var(--theme-accent)] focus:border-[var(--theme-primary)] transition-all">
                        </div>

                        <div>
                            <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-3">Estimasi Waktu</label>
                            <input type="text" name="estimasi_waktu" value="{{ old('estimasi_waktu') }}" placeholder="Contoh: 1-2 Hari"
                                class="w-full glass border border-white/40 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 focus:outline-none focus:ring-4 focus:ring-[var(--theme-accent)] focus:border-[var(--theme-primary)] transition-all">
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="deskripsi" rows="4" placeholder="Jelaskan detail layanan Anda..."
                                class="w-full glass border border-white/40 rounded-3xl px-6 py-4 text-sm font-medium text-slate-600 focus:outline-none focus:ring-4 focus:ring-[var(--theme-accent)] focus:border-[var(--theme-primary)] transition-all">{{ old('deskripsi') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-4 mt-12 pt-8 border-t border-slate-100">
                        ...
                        <button type="button"
                            onclick="window.confirmPremium({
                                ...
                                message: 'Pastikan detail layanan cetak sudah benar sebelum disimpan.',
                                variant: 'success'
